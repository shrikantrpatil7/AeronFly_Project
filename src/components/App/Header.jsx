          ...
          <div className="row gx-0">
            <div className="col-lg-6">
              <div className="info">
                ...
                <h1 className="mb-30">{ rtl ? 'احفظ' : 'Bill Payment' } <span>{ rtl ? 'كل شئ' : 'Made Easy  With' }</span> { rtl ? 'على التطبيق' : 'AeronPay' }</h1>
                <p className="text">{ rtl ? 'لا يلزم الترميز لإجراء التخصيصات. يحتوي المُخصص المباشر' : 'One App For All Your Financial Needs , Start Your Payment' }<br />{ rtl ? 'على كل ما تحتاجه.' : ' Journey With AeronPay.' }</p>
                <div className="d-flex align-items-center mt-50">
                  ...
                  <a href="https://www.apple.com/app-store" rel="noreferrer" className="btn rounded-pill bg-blue4 fw-bold text-white me-4" target="_blank">
                    <small>  <i className="fab fa-google-play me-2 pe-2 border-end"></i> { rtl ? 'تحميل التطبيق' : 'Google Play' }</small>
                  </a>
...
<video className="banner-video ind-vid-rem purchase-img-light" loop autoPlay muted playsInline>
 <source src="/assets/img/header/home-bann.mp4" type="video/mp4" />
</video> 

<video className="banner-video1 ind-vid-res purchase-img-light" loop autoPlay muted playsInline>
